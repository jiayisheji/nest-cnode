  ...
  for (const filepath of glob.sync(pattern)) {
    ...
    if (result) {
      ...
      if (folder === filename) {
        ...
        entry[folder.split('/').concat(filename).join('-')] = filepath.replace(sourceRoot, './src');
      }
    }
  }
  return entry;
...
function getHtml(chunks) {
  ...
  for (const filepath of glob.sync(pattern)) {
    ...
    if (result) {
      ...
      const entry = folder === template ? folder : folder.split('/').concat(template).join('-');
      options.push({
        template: filepath.replace(sourceRoot, './src'),
        filename: `${viewsPath}/${folder}/${template}.hbs`,
        chunks: filename.endsWith('.template') && [...chunks, entry],
        inject: false,
      });
    } else {
      options.push({
