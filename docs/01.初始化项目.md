...
> 目前安装 `nx` 方式很多，结果也不一样，按上面方式安装，如果有一个 `angular.json`, 你需要把它 `version` 改成 2，然后使用 `vs code` 的 `nrwl.angular-console` 点击 `Generate`，找到 `@nx/workspace:convert-to-nx-project`, 勾选 `all`, 完成工作区迁移，这样就会生成一个 `workspace.json` 文件，这样这个文件只有相关项目和路径映射，大大减少了 `workspace.json` 文件的大小，每个项目都会自动生成一个 `project.json` 文件来管理当前项目的配置。
