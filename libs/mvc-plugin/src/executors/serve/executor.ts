import { ExecutorContext, joinPathFragments, logger, parseTargetString, runExecutor } from '@nrwl/devkit';
import { InspectType, NodeExecutorOptions } from '@nrwl/js/src/executors/node/schema';
...
import { getOptionsForExecutor } from '../../utils/combined-options';
...
async function* serveExecutor(buildOptions: ServeExecutorSchema, context: ExecutorContext) {
  const { buildTargetViewOptions: _viewOptions, ...options } = buildOptions;
...
function calculateResolveMappings(context: ExecutorContext, options: NodeExecutorOptions) {
...
async function runProcess(
  ...
  const subProcess = fork(joinPathFragments(__dirname, 'node-with-require-overrides'), options.args, {
    execArgv,
    stdio: 'inherit',
    env: {
      ...process.env,
...
function getExecArgv(options: NodeExecutorOptions) {
  const args = ['-r', require.resolve('source-map-support/register'), ...options.runtimeArgs];
...
async function* startBuild(options: NodeExecutorOptions, context: ExecutorContext) {
  const combinedOptions = getOptionsForExecutor(options, context);

  yield* webpackExecutor(
    {
      ...combinedOptions,
...
async function killCurrentProcess(uniqueKey: string, options: NodeExecutorOptions, signal = 'SIGTERM') {
  const hashedKey = JSON.stringify([uniqueKey, ...options.args]);
...
function runWaitUntilTargets(options: NodeExecutorOptions, context: ExecutorContext): Promise<{ success: boolean }[]> {
  return Promise.all(
